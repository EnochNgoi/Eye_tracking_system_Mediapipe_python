...
CHIN = [152, 148, 160, 161, 162, 163, 164, 165, 166, 167]  # New: Chin indices
...
head_down_angle_threshold = 3
HEAD_DOWN_ALERT_SECONDS = 5
    ...
    while True:
        ...
        if result.multi_face_landmarks:
            mesh_points = np.array(
                [np.multiply([p.x, p.y], [img_w, img_h]).astype(int) for p in result.multi_face_landmarks[0].landmark]
            )
            ...
            cv.polylines(frame, [mesh_points[CHIN]], True, (0, 255, 0), 1, cv.LINE_AA) # New: Display chin on frame
            ...
            chin_angle = calculate_angle(mesh_points[CHIN[0]], mesh_points[TOP_EYE_IDX], mesh_points[BOTTOM_EYE_IDX])  # New: Calculate chin angle
            if chin_angle > head_down_angle_threshold:
        ...
        # Display the chin angle on the frame
        cv.putText(frame, "Chin Angle: {:.2f} degrees".format(chin_angle), (10, 90),
                   cv.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
